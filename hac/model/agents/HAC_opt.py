...
class HAC_opt(DDPG):
    ...
    def __init__(self, args, facade):
        ...
        self.w = args.w
        self.potential = deepcopy(self.critic)
        self.potential_optimizer = torch.optim.Adam(self.potential.parameters(), lr=args.critic_lr // 10)
...
#         done_mask = torch.FloatTensor(done_mask)
    ...
    def get_hac_loss(self, observation, policy_output, reward, done_mask, next_observation, 
        ...
        if do_actor_update and self.actor_lr > 0:
            ...
            observation_p, policy_output_p, reward_p, done_mask_p, next_observation_p = self.facade.sample_buffer(self.batch_size)
            
            # forward для potential_loss
            tmp_policy_output = self.facade.apply_policy(observation_p, self.actor)
            potential_policy = self.facade.apply_critic(observation_p, tmp_policy_output, self.potential)
            potential_data = self.facade.apply_critic(observation_p, policy_output_p, self.potential)
            
            potential_loss = -potential_policy['q'].mean() + self.w * potential_data['q'].mean()
            ...
            potential_policy_new = self.facade.apply_critic(observation, policy_output, self.potential)
            actor_loss = -critic_output['q'].mean() - potential_policy_new['q'].mean()
            # Optimize the actor 
            actor_loss.backward()
