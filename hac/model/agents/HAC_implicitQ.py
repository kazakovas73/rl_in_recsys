...
#         done_mask = torch.FloatTensor(done_mask)
    ...
    def compute_expectile_value(self, Q_values, tau=0.9):
        """Compute V(s) using expectile regression."""
        diff = Q_values - Q_values.mean()
        weights = torch.where(diff > 0, tau, (1 - tau))  # Asymmetric loss weighting
    ...
    def get_hac_loss(self, observation, policy_output, reward, done_mask, next_observation, 
        ...
        ### **Step 1: Compute Current Q-Value (No Change)**
        inverse_output = self.facade.infer_hyper_action(observation, policy_output, self.actor)
        # inverse_output.update({'state': policy_output['state'], 'action': inverse_output['Z']})
        current_critic_output = self.facade.apply_critic(observation, inverse_output, self.critic)
        current_Q = current_critic_output['q']  # (B,)
        
        ### **Step 2: Compute Expectile Value Function V(s')**
        with torch.no_grad():
            next_policy_output = self.facade.apply_policy(next_observation, self.actor_target)
            target_critic_output = self.facade.apply_critic(next_observation, next_policy_output, self.critic_target)
            next_Q = target_critic_output['q']  # (B,)
            
            # Expectile regression to compute V(s')
            V_next = self.compute_expectile_value(next_Q, tau=0.9)
            
        ### **Step 3: Compute Target Q-Value using V(s') Instead of max(Q)**
        # reward = user_feedback['reward'].view(-1)  # (B,)
        target_Q = reward + self.gamma * (done_mask * V_next).detach()
    
        ### **Step 4: Compute TD Error Using MSE Loss**
        critic_loss = F.mse_loss(current_Q, target_Q).mean()
        
        ### **Step 5: Update Critic**
        if do_critic_update and self.critic_lr > 0:
            self.critic_optimizer.zero_grad()
            critic_loss.backward()
            self.critic_optimizer.step()
