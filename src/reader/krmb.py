...
class KRMBSeqReader(BaseReader):
    ...
    def _sequence_holdout(self):
        ...
        for u in tqdm(self.users):
            ...
            n_train = len(sub_df) - self.val_holdout_per_user - self.test_holdout_per_user
            if n_train < 0.6 * len(sub_df):
                continue
            data['train'].append(list(sub_df.index[:n_train]))
            data['val'].append(list(sub_df.index[n_train:n_train+self.val_holdout_per_user]))
            data['test'].append(list(sub_df.index[-self.test_holdout_per_user:]))
