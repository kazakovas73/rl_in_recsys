...
from src.utils import padding_and_clip, get_onehot_vocab, get_multihot_vocab
...
class KRMBSeqReader(BaseReader):
    ...
    def __init__(
        ...
        - from BaseReader:
            ...
            - n_worker
        '''
        super().__init__(train_file, val_file, test_file, n_worker, data_separator)
        print("initiate KuaiRandMultiBehaior sequence reader")
        self.max_hist_seq_len = max_hist_seq_len
        self.val_holdout_per_user = val_holdout_per_user
        self.test_holdout_per_user = test_holdout_per_user

        # read data_file
        print(f"Loading data files")
        self.log_data = pd.read_table(train_file, sep=data_separator)

        print("Load item meta data")
        item_meta_file = pd.read_csv(item_meta_file, sep=meta_file_sep)
        self.item_meta = item_meta_file.set_index('video_id').to_dict('index')
        
        print("Load user meta data")
        user_meta_file = pd.read_csv(user_meta_file, sep=meta_file_sep)
        ...
        self.user_history = {uid: list(
            self.log_data[self.log_data['user_id'] == uid].index) for uid in self.users}
        ...
        self.selected_item_features = [
            'video_type', 'music_type', 'upload_type', 'tag']
        ...
        self.user_vocab = get_onehot_vocab(
            user_meta_file, self.selected_user_features)
        
        self.item_vocab = get_onehot_vocab(
            item_meta_file, self.selected_item_features[:-1])
        ...
        self.padding_response = {resp: 0. for i,
                                 resp in enumerate(self.response_list)}
        self.response_neg_sample_rate = self.get_response_weights()

        # {'train': [row_id], 'val': [row_id], 'test': [row_id]}
        self.data = self._sequence_holdout()

    def _sequence_holdout(self):
        '''
        Holdout validation and test set from log_data
        '''
        print(f"sequence holdout for users (-1, {self.val_holdout_per_user}, {self.test_holdout_per_user})")
        if self.val_holdout_per_user == 0 and self.test_holdout_per_user == 0:
            return {"train": self.log_data.index, "val": [], "test": []}
        data = {"train": [], "val": [], "test": []}
        for u in tqdm(self.users):
            sub_df = self.log_data[self.log_data['user_id'] == u]
            n_train = len(sub_df) - self.val_holdout_per_user - \
                self.test_holdout_per_user
            if n_train < 0.6 * len(sub_df):
                continue
            data['train'].append(list(sub_df.index[:n_train]))
            data['val'].append(
                list(sub_df.index[n_train:n_train+self.val_holdout_per_user]))
            data['test'].append(
                list(sub_df.index[-self.test_holdout_per_user:]))
    ...
    def __getitem__(self, idx):
        ...
        H_rowIDs = [rid for rid in self.user_history[user_id]
                    if rid < row_id][-self.max_hist_seq_len:]
        history, hist_length, hist_meta, hist_response = self.get_user_history(
            H_rowIDs)
    ...
    def get_user_history(self, H_rowIDs):
        ...
        if L == 0:
            ...
            hist_meta = {}
            for f in self.selected_item_features:
                padding = [self.padding_item_meta[f]
                           for i in range(self.max_hist_seq_len - L)]
                real_hist = [v_dict[f'if_{f}'] for v_dict in meta_list]
                # {if_{feature_name}: (max_H, feature_dim)}
                hist_meta[f'if_{f}'] = np.concatenate(
                    padding + real_hist, axis=0)
            # {resp_type: (max_H,)}
            history_response = {}
            for resp in self.response_list:
                padding = np.array([self.padding_response[resp]]
                                   * (self.max_hist_seq_len - L))
                real_resp = np.array(H[resp])
                history_response[resp] = np.concatenate(
                    [padding, real_resp], axis=0)
    ...
    def get_statistics(self):
        ...
        stats["data_size"] = [len(self.data['train']), len(
            self.data['val']), len(self.data['test'])]
        ...
        stats["user_feature_dims"] = {
            f: len(list(v_dict.values())[0]) for f, v_dict in self.user_vocab.items()}
        stats["item_features"] = self.selected_item_features
        stats["item_feature_dims"] = {
            f: len(list(v_dict.values())[0]) for f, v_dict in self.item_vocab.items()}
