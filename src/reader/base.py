...
class BaseReader(Dataset):
    ...
    def __init__(self, train_file, val_file, test_file, n_worker, data_separator):
        ...
        self.data['val'] = pd.read_table(val_file, sep=data_separator) if len(val_file) > 0 else self.data['train']
        self.data['test'] = pd.read_table(test_file, sep = data_separator) if len(test_file) > 0 else self.data['val']
