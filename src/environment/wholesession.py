...
class KREnvironment_WholeSession_GPU(BaseRLEnvironment):
    ...
    def __init__(
        ...
        '''
        super(KREnvironment_WholeSession_GPU, self).__init__(max_step_per_episode, initial_temper, device, model_path, model, reader_stats, from_load)
        ...
        # assert (reader == 'KRMBSeqReader' or reader == 'MLSeqReader') and 'KRMBUserResponse' in model
        
        # print("Load user sequence reader")
        # reader, reader_args = self.get_reader(uirm_log_path) # definition in base

        self.reader = reader
        print(self.reader.get_statistics())
        
        # print("Load immediate user response model")
        # uirm_stats, uirm_model, uirm_args = self.get_user_model(uirm_log_path, device) # definition in base
        ...
        self.response_weights = torch.tensor(list(self.reader.get_response_weights().values())).to(torch.float).to(device)
        if single_response:
