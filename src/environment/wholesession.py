...
from src.environment.base import BaseRLEnvironment
...
class KREnvironment_WholeSession_GPU(BaseRLEnvironment):
    ...
    def __init__(
            self,
            max_step_per_episode, initial_temper, device,
        ...
        '''
        super(KREnvironment_WholeSession_GPU, self).__init__(max_step_per_episode,
                                                             initial_temper, device, model_path, model, reader_stats, from_load)

        self.uirm_log_path = uirm_log_path
        self.slate_size = slate_size
        self.episode_batch_size = episode_batch_size
        self.rho = item_correlation
        self.single_response = single_response

        self.reader = reader

        uirm_stats = reader_stats

        self.immediate_response_stats = uirm_stats
        self.immediate_response_model = model
        ...
        self.response_weights = torch.tensor(
            list(self.reader.get_response_weights().values())).to(torch.float).to(device)
        if single_response:
        ...
        self.candidate_iids = torch.tensor(
            [reader.item_id_vocab[iid] for iid in reader.items]).to(self.device)

        # item meta: {'if_{feature_name}': (n_item, feature_dim)}
        candidate_meta = [reader.get_item_meta_data(
            iid) for iid in reader.items]
        ...
        self.candidate_item_encoding = item_enc.view(
        ...
        self.immediate_response_model.to(device)
        self.immediate_response_model.device = device
    ...
    def get_candidate_info(self, feed_dict, all_item=True):
        ...
        if all_item:
            ...
            candidate_info.update(
                {k: v[indices] for k, v in self.candidate_item_meta.items()})
    ...
    def reset(self, params={'empty_history': True}):
        ...
        self.current_step = torch.zeros(
            self.episode_batch_size).to(self.device)
        self.current_sample_head_in_batch = BS

        # user temper for leave model
        self.current_temper = torch.ones(self.episode_batch_size).to(
            self.device) * self.initial_temper
    ...
    def step(self, step_dict):
        ...
        with torch.no_grad():
            ...
            # (B,)
            # this will also change self.current_temper
            done_mask = self.get_leave_signal(None, None, response_dict)
            ...
            if n_leave > 0:
                final_steps = self.current_step[done_mask].detach(
                ).cpu().numpy()
                ...
                if self.current_sample_head_in_batch + n_leave < self.episode_batch_size:
                    ...
                    self.sample_batch = self.get_observation_from_batch(
                        sample_info)
                    for obs_key in ['user_profile', 'user_history']:
                        for k, v in self.sample_batch[obs_key].items():
    ...
    def get_response(self, step_dict):
        ...
        point_scores = torch.sigmoid(
            behavior_scores) - corr_factor.view(B, self.slate_size, 1) * self.rho
    ...
    def get_intra_slate_similarity(self, action_item_encoding):
        ...
        pair_similarity = torch.mean(action_item_encoding.view(
            B, L, 1, d) * action_item_encoding.view(B, 1, L, d), dim=-1)
    ...
    def get_leave_signal(self, user_state, action, response_dict):
        ...
        '''
        # (B, slate_size, n_feedback)
        point_reward = response_dict['immediate_response'] * \
            self.response_weights.view(1, 1, -1)
        # (B, slate_size)
    ...
    def update_observation(self, user_state, action, response, done_mask, update_current=True):
        ...
        for k, candidate_meta_features in self.candidate_item_meta.items():
            ...
            # (B, max_H, feature_dim)
            previous_meta = old_history[f'history_{k}'].view(
                self.episode_batch_size, max_H, -1)
            new_history[f'history_{k}'] = torch.cat((previous_meta, meta_features), dim=1)[
                :, -max_H:, :].view(self.episode_batch_size, -1)
        # history item responses
        for i, R in enumerate(self.immediate_response_model.feedback_types):
            k = f'history_{R}'
            new_history[k] = torch.cat(
                (old_history[k], response[:, :, i]), dim=1)[:, -max_H:]
        if update_current:
    ...
    def create_observation_buffer(self, buffer_size):
        ...
        for f, f_dim in self.observation_space['user_feature_dims'].items():
            observation['user_profile'][f'uf_{f}'] = torch.zeros(
                buffer_size, f_dim).to(torch.float).to(self.device)
